...
describe('evaluated spans', () => {
	...
	async function spans(content: string): Promise<Array<{ text: string; value: unknown }>> {
		const found = await evaluator.evaluatedSpans(configPath, content, process.cwd());
		return found.map(span => ({ text: content.slice(span.start, span.end), value: runtimeValueToPlain(span.value) }));
	}
	...
	it('marks what had to be worked out and nothing the source already spells out', async () => {
		...
		assert.deepEqual(await spans(content), [
			{ text: 'name', value: 'svc-prod' },
			{ text: '"svc-${local.env}"', value: 'svc-prod' },
			{ text: 'local.env', value: 'prod' },
			{ text: 'shouted', value: 'PROD' },
			{ text: 'upper(local.env)', value: 'PROD' },
			{ text: 'local.env', value: 'prod' },
			{ text: 'owner', value: { team: 'prod' } },
			{ text: 'team', value: 'prod' },
			{ text: 'local.env', value: 'prod' },
			{ text: 'sum', value: 3 },
			{ text: '1 + 2', value: 3 },
			{ text: 'cidr', value: '10.11.0.0/16' },
			{ text: 'local.cidr', value: '10.11.0.0/16' }
		]);
	});
	...
	it('gives each expression its own value rather than that of the first operand it starts with', async () => {
		...
		assert.deepEqual(found.find(span => span.text === 'local.flag ? "yes" : "no"'), { text: 'local.flag ? "yes" : "no"', value: 'yes' });
		assert.deepEqual(found.find(span => span.text === '2 * 21'), { text: '2 * 21', value: 42 });
	});
	...
	it('leaves out an expression that cannot be evaluated in the file scope, and keeps the one around it', async () => {
		...
		assert.deepEqual(await spans(content), [
			{ text: 'loud', value: ['A', 'B'] },
			{ text: '[for name in local.names : upper(name)]', value: ['A', 'B'] },
			{ text: 'local.names', value: ['a', 'b'] }
		]);
	});
